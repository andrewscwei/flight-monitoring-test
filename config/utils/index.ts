...
export function getRoutesFromDir(dir: string, baseDir: string = dir): RouteData[] {
  const pages: string[] = fs.readdirSync(dir);
  let out: RouteData[] = [];
  ...
  pages.forEach((fileName: string) => {
    ...
    const tmp: string[] = path.join(dir, basename).replace(baseDir, '').split('/').filter((val: string) => val);
...
export function getLocalizedRoutesFromDir(dir: string, whitelistedLocales: string[]): RouteData[] {
  // Generate routes based on the pages directory.
  const routes = getRoutesFromDir(dir);
  let localizedRoutes: RouteData[] = [];
